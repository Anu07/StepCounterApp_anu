...
class BmiCalculatorActivity : BaseActivity<SignInViewModel>(), View.OnClickListener {
    ...
    override fun onCreate() {
        ...
        val pickerLayoutManager_ht =
            PickerLayoutManager(this, PickerLayoutManager.HORIZONTAL, false)
            ...
            .observe(this, Observer { mUser ->
                if (mUser.status == 200) {
                    if (intent.hasExtra("inApp")) {
                        ...
                        var userData = SharedPreferencesManager.getUserObject(this).data
                        var responseLogin = LoginResponseJ()
                        var loginData = Data(
                            mUser.data._id,
                            mUser.data.basicFlag,
                            mUser.data.dob,
                            userData.elciesCallbackurl,
                            userData.elciesUserId,
                            mUser.data.email,
                            mUser.data.firstName,
                            mUser.data.image,
                            userData.elciesConnected,
                            mUser.data.lastName,
                            mUser.data.rewardFlag,
                            mUser.data.username,
                            userData.wearableDevice
                        )
                        responseLogin.data = loginData
                        SharedPreferencesManager.saveUserObject(
...
//                      val options = ActivityOptions.makeSceneTransitionAnimation(this@SignInActivity)
        ...
        if (intent.hasExtra("inApp")) {
            ...
            if (SharedPreferencesManager.hasKey(this@BmiCalculatorActivity, "UpdatedUser")) {
                var mSavedUser =
                    SharedPreferencesManager.getUpdatedUserObject(this@BmiCalculatorActivity)
                ...
                val dialog =
                    BMIResultDialog(mContext,
                        ...
                        InterfacesCall.Callback {
                            SharedPreferencesManager.saveBmiObject(
                                this@BmiCalculatorActivity,
                                BMIObject(height, h, weight.toInt(), w)
                            )
                            if (!intent.hasExtra("inApp")) {
