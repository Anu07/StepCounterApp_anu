        ...
        if (Utils.retryInternet(HayaTechApplication.applicationContext())) {
            call!!.add_basic_info(login).enqueue(object : Callback<BMIinfoResponse> {
                ...
                override fun onResponse(
                    ...
                    when {
                        response!!.body()!!.status == 200 -> mBmiResponseModel!!.value = response.body()!!
                        response.code() == 405 -> {
                            Toast.makeText(
                                HayaTechApplication.applicationContext(),
                                "User doesn't exist",
        ...
        if (Utils.retryInternet(HayaTechApplication.applicationContext())) {
            call!!.check_username(userName).enqueue(object : Callback<BasicInfoResponse> {
                ...
                override fun onResponse(
                    ...
                    when {
                        response!!.code() == 400 -> {
                            var responseBody = BasicInfoResponse()
                            responseBody.message =
                                "username must be 3 to 15 characters long and it may be alphanumeric."
                            mBasicResponseModel!!.value = responseBody
                        }
                        response.code() == 405 -> {
                            Toast.makeText(
                                HayaTechApplication.applicationContext(),
            ...
            call!!.getSelectedRewardsCategories(
                ...
                .enqueue(object : Callback<SelectedRewardCategories> {
                    ...
                    override fun onResponse(
                        ...
                        when {
                            response!!.code() != 400 -> mSelectedRewardsCategoriesResponseModel!!.value =
                                response.body()!!
                            response.code() == 405 -> {
                                Toast.makeText(
                                    HayaTechApplication.applicationContext(),
