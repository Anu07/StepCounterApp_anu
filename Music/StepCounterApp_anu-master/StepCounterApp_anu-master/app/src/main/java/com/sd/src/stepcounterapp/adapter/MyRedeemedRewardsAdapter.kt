...
class MyRedeemedRewardsAdapter(mContext: Context?, mData: MyRedeemedResponse) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.textView.text = mData.data[position].name.capitalize()
        holder.textShort.text = HtmlCompat.fromHtml(mData.data[position].description.capitalize(), HtmlCompat.FROM_HTML_MODE_LEGACY)
        holder.tokenTxt.visibility = View.VISIBLE
        holder.tokenTxt.text = mData.data.get(position).token.toString() + " TKNS"
        /* holder.imageView.setOnClickListener {
             mListener!!.onItemClick(position, item)
         }*/
        Picasso.get().load(IMG_URL + "" + mData.data.get(position).image)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val view =
            LayoutInflater.from(mContext).inflate(R.layout.item_mychallenges_list, parent, false)
