...
//                                        val options = ActivityOptions.makeSceneTransitionAnimation(this@SignInActivity)
                    ...
                    showPopupProgressSpinner(false)
                    Toast.makeText(this@BasicInfoActivity, mUser.message, Toast.LENGTH_LONG).show()
                }
            })
...
//                                        val options = ActivityOptions.makeSceneTransitionAnimation(this@SignInActivity)
    ...
    override fun initListeners() {
        if (SharedPreferencesManager.hasKey(this@BasicInfoActivity, VAR_USEROBJECT)) {
    ...
    private fun setTermsPolicyClick() {

        var ss = SpannableString("I agree to the Terms of Service and Privacy Policy.")
        val clickableTnCSpan = object : ClickableSpan() {
        ...
        val clickablePrivacySpan = object : ClickableSpan() {
            override fun onClick(widget: View) {
                // We display a Toast. You could do anything you want here.
                startActivity(
                    Intent(
        ...
        ss.setSpan(clickableTnCSpan, 15, 30, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        ss.setSpan(clickablePrivacySpan, 36, 50, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

        policyTxt.highlightColor = Color.BLUE
        policyTxt.setTextColor(resources.getColor(R.color.colorBlack))
        policyTxt.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12f)
        policyTxt.text = ss
        policyTxt.movementMethod = LinkMovementMethod.getInstance()
    ...
    private fun validate(): Boolean {
        if (!emailTxt.nonEmpty()) {
            ...
            Toast.makeText(
                this@BasicInfoActivity,
                resources.getString(R.string.pp_error),
                Toast.LENGTH_LONG
            ).show()
            return false
...
//                profileImg.setImageURI(selectedImage)
    ...
    fun getFiletoServer(): Part {
        ...
        if (croppedImage != null) {
            var requestBody =
                RequestBody.create(MediaType.parse("multipart/form-data"), croppedImage)
    ...
    private fun setSpanOnLink(ss: SpannableString, link: String, cs: ClickableSpan) {
        var text = ss.toString()
        var start = text.indexOf(link)
